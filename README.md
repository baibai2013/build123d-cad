...
**自然语言 → 零件设计 → 装配预览 → 运动仿真 → 工业级输出。一句话，一个完整 CAD 工程。**
...
融合 Dave Cowden 建模哲学「像机械师思考」与 Peter Corke 仿真哲学「Learn by doing」，<br>
覆盖零件建模、装配爆炸动画、曲面/关节/安装实战、制造工艺验证、FK/IK/步态/URDF/PyBullet 全链路。<br>
25+ 可运行示例、8 大类参考文档、10 个工具脚本。<br>
一句话描述你想要的零件，从草图到运动仿真一步到位。
...
### 运动仿真 (`assets/simulation/`)

| # | 示例 | 难度 | 核心技法 |
|---|------|------|----------|
| 21 | [FK 腿链](assets/simulation/21_fk_leg_chain.py) | ★★ | DH 齐次变换 + OCP 可视化 |
| 22 | [IK 单腿](assets/simulation/22_ik_single_leg.py) | ★★★ | 解析 IK + 双构型对比 |
| 23 | [工作空间](assets/simulation/23_workspace_cloud.py) | ★★ | FK 点云 + 可达性可视化 |
| 24 | [步态生成器](assets/simulation/24_gait_generator.py) | ★★★★ | 贝塞尔轨迹 + IK + OCP 动画 |
| 25 | [URDF 导出](assets/simulation/25_urdf_export.py) | ★★★ | build123d → URDF + STL |

---

## 工具脚本

`scripts/` 目录包含 10 个实用工具，按功能分类：
...
### 仿真 (`scripts/simulation/`)

| 脚本 | 功能 |
|------|------|
| [`export_urdf.py`](scripts/simulation/export_urdf.py) | STEP → URDF 自动导出（遍历 Compound） |
| [`pybullet_preview.py`](scripts/simulation/pybullet_preview.py) | PyBullet URDF 加载 + 步态预览 |
...
│   ├── peter-corke/                  # Peter Corke 仿真哲学
│   │   └── simulation-philosophy.md  #   「Learn by doing」+ DH 标准
│   └── simulation/                   # 运动仿真
│       ├── forward-kinematics.md     #   正运动学（DH + 齐次变换）
│       ├── inverse-kinematics.md     #   逆运动学（解析/数值/工作空间）
│       ├── gait-planning.md          #   步态规划（贝塞尔轨迹 + IK）
│       ├── urdf-export.md            #   URDF 导出（build123d → URDF）
│       └── pybullet-quickstart.md    #   PyBullet 快速入门
├── assets/                           # 25+ 可运行示例
│   ├── parts/                        #   13 个零件（01~13）
│   ├── assembly/                     #   装配预览 + 爆炸动画
│   ├── surface/                      #   曲面建模示例
│   ├── joints/                       #   关节装配示例
│   ├── mounting/                     #   安装实战示例
│   └── simulation/                   #   运动仿真示例（FK/IK/步态/URDF）
└── scripts/                          # 10 个工具脚本
    ├── validate/                     #   几何验证 + 碰撞检测
    ├── analysis/                     #   参数提取 + STEP 信息 + 质量分析
    ├── export/                       #   批量导出 + 打印导出
    ├── assembly/                     #   爆炸动画生成器
    └── simulation/                   #   URDF 导出 + PyBullet 预览
```
